...
static void	apply_move(t_mlx *mlx)
{
	...
	if (!(mlx->elements.exit.x == mlx->elements.player.x && mlx->elements.exit.y == mlx->elements.player.y))
	...
	if (!(mlx->elements.exit.x == mlx->elements.player.x && mlx->elements.exit.y == mlx->elements.player.y))
		mlx->file.point[mlx->elements.player.y][mlx->elements.player.x] = PLAYER;
	mlx_put_image_to_window(
		mlx->ptr,
		mlx->window.ptr,
...
int	move(t_mlx *mlx, int y, int x)
{
	...
	if (c == COLLECTIBLE)
		get_collectible(mlx, mlx->elements.player.current_y, mlx->elements.player.current_x);
